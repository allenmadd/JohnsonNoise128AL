---
title: "Experiment 3: kB for Gain 150"
...
```{r, echo=FALSE}
Temperature2Error<-Temperature2Error/1000000
```
...
```{r}
library(ggplot2)
qplot((Temperature2),(y2))+geom_errorbar(aes(x=(Temperature2), ymin=(y2-Temperature2Error2), ymax=(y2+Temperature2Error2)), width=0.25)+
  geom_smooth(method="lm", se=FALSE, fullrange=TRUE, level=0.95)+labs(title = "Temperature vs Voltage/GR", x= "Temperature", y = "V^2/ (4*G*R)")
...
```{r}
shapiro.test(y) #not normally distributed

yprime <- sqrt(y +.5)

shapiro.test(yprime) #transformed data is normal! will use t-test on this transformed data.

```

You have to divide by 100 for some reason in all of the results. Our result for the slope is 1.381*10^-23 with an error
