...
#gamma anad weibull both didnt work i will try poisson
...
fit2 <- lm(yRange~poly(xRange,2,raw=TRUE))
fit3 <- lm(yRange~poly(xRange,3,raw=TRUE))
fit4 <- lm(yRange~poly(xRange,4,raw=TRUE))
fit5 <- lm(yRange~poly(xRange,5,raw=TRUE))
...
#G= integrate((gF_list)^2/1+(2*pi*), lower=3,upper = 10)
#integrate()

#G1k=integrate(((stupidFit^2)/(1+2*pi*capacitance*k1*f)^2), lower=3, upper=10)

#http://spa-mxpweb.spa.umn.edu/resources/ExpWriteups/LabManNoise.pdf
