...
```{r}
library(ggplot2)
qplot(unlist(gain$Frequency[24:80]),unlist(gain$Volts[24:80]))+geom_errorbar(aes(x=unlist(gain$Frequency[24:80]), ymin=unlist(gain$Volts[24:80]-gainerror$Volts[24:80]), ymax=unlist(gain$Volts[24:80]+gainerror$Volts[24:80])), width=0.25)+labs(title = "g(F)", x= "Frequency (Hz)", y = "Volts (V)")
